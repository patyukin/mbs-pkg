...
gen-pb:
	...
	# credit_v1
	rm -rf pkg/proto/credit_v1
	mkdir -p pkg/proto/credit_v1
	protoc --proto_path=api/v1 --proto_path=vendor.protogen \
		--go_out=pkg/proto/credit_v1 --go_opt=paths=source_relative \
		--plugin=protoc-gen-go=bin/protoc-gen-go \
		--go-grpc_out=pkg/proto/credit_v1 --go-grpc_opt=paths=source_relative \
		--plugin=protoc-gen-go-grpc=bin/protoc-gen-go-grpc \
		--validate_out=lang=go:pkg/proto/credit_v1 --validate_opt=paths=source_relative \
		--plugin=protoc-gen-validate=bin/protoc-gen-validate \
		api/v1/credit.proto

	# report_v1
	rm -rf pkg/proto/report_v1
	mkdir -p pkg/proto/report_v1
	protoc --proto_path=api/v1 --proto_path=vendor.protogen \
		--go_out=pkg/proto/report_v1 --go_opt=paths=source_relative \
		--plugin=protoc-gen-go=bin/protoc-gen-go \
		--go-grpc_out=pkg/proto/report_v1 --go-grpc_opt=paths=source_relative \
		--plugin=protoc-gen-go-grpc=bin/protoc-gen-go-grpc \
		--validate_out=lang=go:pkg/proto/report_v1 --validate_opt=paths=source_relative \
		--plugin=protoc-gen-validate=bin/protoc-gen-validate \
		api/v1/report.proto
